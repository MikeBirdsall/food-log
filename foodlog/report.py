...
def datespan(d1, d2):
    """ Return sequence of date strings from d1 to d2 inclusive """
    if d1 < d2:
        dates = (d1 + timedelta(n) for n in range((d2-d1).days+1))
    else:
        dates = (d1 + timedelta(n) for n in range(0, (d2-d1).days+1, -1))
