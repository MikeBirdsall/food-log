...
def edit_url(dish):
    """ Return link to detail for dish """

    label = ellipse_truncate(dish.description, default="No Description Yet")
    return '<a href="./detail.py?id={id}">{label}</a>'.format(
...
class FullTextSearch(object):
    ...
    def form_entry(self):
        ...
        for course, score in TextSearchEngine(DB_FILE, searchstring).results():
            print(SEARCH_COURSE_TEMPLATE.format(**self.course_dict(course), score=score))
